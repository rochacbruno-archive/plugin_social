...
def plugin_social_friends():
    ...
    friends = db(u.id==l.source)(l.target==me).select(orderby=u.first_name|u.last_name)
    requests = db(u.id==l.target)(l.source==me).select(orderby=u.first_name|u.last_name)
    return locals()
...
def plugin_social_friend_ids(user_id=auth.user_id):
    return [row.id for row in db(db.plugin_social_link.source==user_id)\
