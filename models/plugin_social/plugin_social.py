...
def plugin_social_friend_button(user_id,
                                name=T('Friend'),
                                action='request',
                                replace=T('pending')):
    jquery = ("var x=this;jQuery.post('%s',function(d,t,r)"
                        "{jQuery(x).parent().html('%s');})")
    url = (URL('plugin_social', 'callback', args=(action, user_id)), replace)
    return TAG.button(name,
                      _onclick=jquery % url)

def plugin_social_accept_button(user_id):
    return plugin_social_friend_button(user_id,
                                       T('Accept'), 'accept', T('Accepted'))

def plugin_social_deny_button(user_id):
    return plugin_social_friend_button(user_id,
                                       T('Deny'), 'deny', T('Denied'))

def plugin_social_revoke_button(user_id):
    return plugin_social_friend_button(user_id,
                                       T('Revoke'), 'revoke', T('revoked'))
...
def plugin_social_search():
    ...
    if form.accepts(request):
        ...
        query = reduce(lambda a, b: a & b,
          [u.first_name.contains(k) | u.last_name.contains(k) for k in tokens])
        people = db(query).select(orderby=u.first_name|u.last_name)
    else:
        people = []
...
def plugin_social_friends():
    ...
    friends = db(u.id==l.source_user)(l.target_user==me)\
        .select(orderby=u.first_name|u.last_name)
    requests = db(u.id==l.target_user)(l.source_user==me)\
        .select(orderby=u.first_name|u.last_name)
    return dict(u=u, l=l, me=me, friends=friends, requests=requests)
